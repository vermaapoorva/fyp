...
def write_data_to_tar(indices, shard_file_name, dataset_directory, start_index):
    ...
    count = 0
    for i in indices:
        print(f"Count for {start_index}: {count}")
...
pool = mp.Pool(processes=8)  # Use multiprocessing pool
...
# results = []
# indices_for_shards = np.array_split(indices, num_of_shards)
# for i, shard_filename in enumerate(shard_filenames):
#     results.append(write_data_to_tar(indices_for_shards[i], shard_filename, dataset_directory, start_indices[i]))
