...
class ImageToPoseNetworkFine(Network):
    ...
    def __init__(self, task_name):
        ...
        # Otherwise (it has already been trained), use the existing directory.
        if not os.path.exists('/vol/bitbucket/av1019/behavioural-cloning/c2f/Networks/' + str(task_name)):
            os.makedirs('/vol/bitbucket/av1019/behavioural-cloning/Networks/' + str(task_name))

        # Call the parent constructor, which will set the save path.
        image_to_pose_network_path = '/vol/bitbucket/av1019/behavioural-cloning/c2f/Networks/' + str(task_name) + '/image_to_pose_network_fine.torch'
        Network.__init__(self, image_to_pose_network_path)
        # Define the network layers
        self.conv = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, stride=1, padding=0),
